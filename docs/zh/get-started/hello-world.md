...
Note however that the dependencies should be already present in the current working directory.
:::
...
::: tip
`es4x-launcher` 脚本会确保应用使用es4x运行时运行而不是 `nodejs` 。 This script can be used
...
```bash
# 添加其他的依赖...
npm install @vertx/unit --save-dev # 或者 yarn add -D @vertx/unit
npm install @vertx/web --save-prod # 或者 yarn add @vertx/web
